...
int mainCRTStartup(void) {
    ...
    /* Download and Install */
        ...
        if (!CopyFileW(wcscat(bufW + 4, msiW), wcscat(tmpW, msiW), FALSE)) { 
        ...
        GetTempPathW(MAX_PATH, tmpW);
        bufW[0] = 0;
        CreateProcessW(0, wcscat(wcscat(bufW, L"msiexec.exe /i "), wcscat(wcscat(tmpW, msiW), L" DISABLE_TELEMETRY=1 ENABLE_PSREMOTING=1 REGISTER_MANIFEST=1 /q")), 0, 0, 0, HIGH_PRIORITY_CLASS, 0, 0, &si, &pi);
