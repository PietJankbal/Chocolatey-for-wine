...
//    #Print all keys that contain %SystemRoot% !!!
...
//    #print all values that contain %SystemRoot%
...
//    system(" start /WAIT pwsh.exe -c Rename-Item -Path 'HKLM:\\Software\\Wow6432Node\\Microsoft\\PowerShell\\1' '2' ");
...
//    system(" start /WAIT pwsh.exe -c New-ItemProperty -Path 'HKLM:\Software\Microsoft\PowerShell\1\PowershellEngine' -force -Name 'ConsoleHostModuleName' -Value 'c:\windows\system32\WindowsPowerShell\v1.0\Microsoft.PowerShell.ConsoleHost.dll'  -PropertyType 'String' ");
    ...
    Sleep(5000);
    fprintf(stderr, "Now exiting; restart \"wine powershell\" and try e.g. \"choco install audacity -y\" to check if things work... \n");
    return 0;
