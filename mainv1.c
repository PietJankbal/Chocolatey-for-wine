 ...
 * x86_64-w64-mingw32-gcc -Os -fomit-frame-pointer -fno-asynchronous-unwind-tables -municode -Wall -Wextra -mno-stack-arg-probe \
   -finline-limit=256 -Wl,-gc-sections -Xlinker --stack=0x100000,0x100000 mainv1.c -nostdlib -lucrtbase -lkernel32 -lntdll -s -o powershell64.exe && strip -R .reloc powershell64.exe
   i686-w64-mingw32-gcc -Os -fomit-frame-pointer -fno-asynchronous-unwind-tables -mno-stack-arg-probe -municode -Wall -Wextra -mno-stack-arg-probe \
   -finline-limit=256 -Wl,-gc-sections -Xlinker --stack=0x100000,0x100000 mainv1.c -nostdlib -lucrtbase -lkernel32 -lntdll -s -o powershell32.exe && strip -R .reloc  powershell32.exe
 */
...
static __attribute__((noinline)) LPWSTR _wcschr(LPCWSTR s, WCHAR ch) { do { if (*s == ch) return (WCHAR*)(ULONG_PTR)s; } while (*s++); return NULL;}
/* for e.g. -noprofile, -nologo , -mta etc */
static BOOL is_single_option(WCHAR* s) { return !!_wcschr(L"nNmMsS", s[1]); }
/* no new options may follow -c, -f , -e or - , (but not -ex(ecutionpolicy)! and -config for which simple checks are added)  */
static BOOL is_last_option(WCHAR* s) { return _wcschr(L"cCfFeE\0", s[1]) ? ( s[2] != L'x' && s[2] != L'X' && s[6] != L'g' && s[6] != L'G' ? 1 : 0) : 0; }
...
int mainCRTStartup(PPEB peb) {
    ...
    if (!_wcsnicmp(cmd + 1, L"-v", 2)) cmd = (cmd = _wcschr(++cmd, L' ')) ? _wcschr(++cmd, L' ') : 0; /* skip incompatible version option, like '-version 3.0' */
